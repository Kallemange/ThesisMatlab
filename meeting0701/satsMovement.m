...
colorstring = zeros(20,3);
for i = 1:20
   colorstring(i,:) = rand(1,3);
end
...
for i=1:length(x.satPos.elAz)
    ...
    if(~isempty(xs)&&str2num(satLegend(i))<=32)
        ...
        if x.satID(i)<=32
            polarplot(deg2rad(xs(:,2)), xs(:,3), '*', 'Color', colorstring(i,:));
        end

        hold on
end
polarplot(0,90) %To get it all in 0-90 values instead of max elevation
satLegend(satLegendRm)=[];
legend(satLegend);
...
hold on
L=6*3600;
t=x.tVec(1):x.tVec(end);
Xs.pos=zeros(length(t),3);
...
for j=1:length(eph)
    ...
    idx=el>0;
    plot((t(idx)-t(1))/600,el(idx), 'LineWidth', 5, 'Color', colorstring(j,:));
...
saveas(fig5, 'Figures/skyplot/elev', 'epsc')
saveas(fig5, 'Figures/skyplot/elev', 'fig')
saveas(fig1, 'Figures/skyplot/elAz', 'fig')
saveas(fig1, 'Figures/skyplot/elAz', 'epsc')
