...
numVec   = ['0', '1', '2'];
fig=figure;
sgtitle(strcat("True distance ", num2str(dist),"m in ", dir,"-direction"))
for i=1:3
    subplot(4,1,i)
    ...
    xlabel(strcat(labelVec(i,:), ' mean 1:=0[m], \sigma_1= ',num2str(round(sqrt(var1),2)), ...
                                 ', mean 2= ', num2str(round(mu2,2)), '[m]', ...
                                ', \sigma_2= ', num2str(round(sqrt(var2),2))))
end
...
xlabel(strcat('Relative distance in N-E-D directions, mean: ', ...
    ...
    ' \sigma: ', ...
    num2str(round(sqrt(var(distNED(:,1))),2)), ', ', ...
    num2str(round(sqrt(var(distNED(:,2))),2)), ', ', ...
    num2str(round(sqrt(var(distNED(:,3))),2))))

if saveToFile
    figname=strcat("GPShist",num2str(dist),"m",dir);
    saveas(fig, strcat('Figures/',figname), 'epsc')
    saveas(fig, strcat('Figures/',figname), 'fig')
end
