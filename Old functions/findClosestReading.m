function [t1idx t2idx]=findClosestReading(r1, r2)
...
t1first     =find(t0<=[r1(:).ToW], 1, 'first');
t1last      =find(t_end>=[r1(:).ToW], 1, 'last');
t2first     =find(t0<=[r2(:).ToW], 1, 'first');
t2last      =find(t_end>=[r2(:).ToW], 1, 'last');
